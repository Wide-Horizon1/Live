...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0,
        ...
        def build_result_dict(report, query_res_lines):
            ...
            print('result ', rslt)
            print('lines ', query_res_lines)

            for query_res in query_res_lines:
                for i in range(len(periods)):
                    period_key = f'period{i}'
                    rslt[period_key] += query_res[period_key]
            print('query res', query_res_lines[
                0])
            query_res = query_res_lines[0]
            partner_credit_limit = 0
            if query_res.get('grouping_key'):
                partner_credit_limit = self.env['res.partner'].search \
                    ([('id', '=', query_res_lines[0]['partner_id'][0])]).c_credit_limit
            if current_groupby == 'id':
                ...
                print('partner_id', query_res['partner_id'])
                rslt.update({
                ...
                print('partner ', query_res['partner_id'])
                rslt.update({
                    'due_date': None,
                    ...
                    'has_sublines': False,
                    'credit_limit': partner_credit_limit
                })
            print('rslt', rslt)
            return rslt
        ...
        print('query lines : ', query_res_lines)
