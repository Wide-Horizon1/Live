...
class AccrualHolidaysFromDateDeparture(models.Model):
    ...
    # def calc_total_remaining_time_off(self):
    #     num = 0.0
    #     all_allocations = self.env['hr.leave.allocation'].search([
    #         ('employee_id', '=', self.id),
    #         ('allocation_type', '=', 'accrual'),
    ...
    #     ])
    #     if all_allocations:
    #         print("user accrual and has plan accrual", all_allocations)
    #         level = self.env['hr.leave.accrual.level'].search([
    #             ('accrual_plan_id', '=', all_allocations.accrual_plan_id.id)
    #         ])
    #         if level:
    #             print("all level plan", level)
    #             num = level[0].maximum_leave
    #             print("maximum leave in level 1", num)
    #             if all_allocations.date_from:
    #                 date_to = self.env['hr.employee']
    #                 print("today", all_allocations.date_from)
    #                 print("departure date", date_to.departure_date)
    #                 accrual_period_days = (all_allocations.date_to - all_allocations.date_from).days
    #                 print("days count", accrual_period_days)
    #                 if accrual_period_days != 0:
    #                     test_date = accrual_period_days / 12
    #                     print("how per month", test_date)
    #                     final = num / test_date
    #                     print("maximum leave in level 1 / how per month", final)
    #                     self.total_remaining_time_off = final
    #                 else:
    #                     # Handle the case where accrual_period_days is zero
    #                     self.total_remaining_time_off = 0.0
    #             else:
    #                 # Handle the case where date_from or date_to is not set
    #                 self.total_remaining_time_off = 0.0
    #         else:
    #             # Handle the case where level is not found
    #             self.total_remaining_time_off = 0.0
    #     else:
    #         # Handle the case where no accruals are found
    #         self.total_remaining_time_off = 0.0
